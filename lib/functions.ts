...
export const validateHomePage = (
  getClient: (usePreview: any) => SanityClient,
  path: { path: string }
) => {
  getClient(false)
    .listen(`*[_type == "siteTheme"]`)
    .subscribe(() => {
      axios.post('/api/revalidate', path)
    })
  getClient(false)
    .listen(`*[_type == "header"]`)
    .subscribe(() => {
...
export const validateCareers = (
  ...
  getClient(false)
    .listen(`*[_type == "job"]{location, title, type, details}`)
    .subscribe(() => {
      axios.post('/api/revalidate', path)
    })
